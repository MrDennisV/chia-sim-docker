...
from .utils import coin_name, hexstr_to_bytes, int_to_hex

logger = logging.getLogger("openapi.watcher")


class Watcher:
    def __init__(self, url_or_path: str, chain_key: str, database_uri: str):
    ...
    async def start(self):
        ...
        register_db(self.chain_key, self.database_uri)
        await connect_db(self.chain_key)
        self.db = get_db(self.chain_key)
            ...
            if prev_block and bytes(prev_block.hash) != bytes(block.prev_hash):
                ...
                while check_height:
                    ...
                    if hexstr_to_bytes(bc['header_hash']) == bytes(db_block.hash):
                        ...
                        check_height -= 1
                start_height = check_height  # will +1 at end of loop
...
async def main(networks: List[str] = None):
    ...
    for row in settings.SUPPORTED_CHAINS.values():
        ...
        chain_key = int_to_hex(row['id'])
        tasks.append(Watcher(row['rpc_url_or_chia_path'], chain_key, row['database_uri']).start())
