...
from .db import (
    ...
    NftMetadata,
    SingletonSpend,
    delete_singleton_spend_by_id,
    get_address_sync_height,
    get_db,
    get_latest_tx_block_number,
    get_nft_metadata_by_hash,
    get_singelton_spend_by_id,
    save_address_sync_height,
    save_asset,
    save_metadata,
    save_singleton_spend,
)
...
async def handle_coin(address, coin_record, parent_coin_spend, db):
    ...
    if nft_info is not None:
        ...
        curried_params = {
            ...
            'transfer_program': to_hex(bytes(uncurried_nft.transfer_program) if uncurried_nft.transfer_program else None),  # noqa: E501
...
async def get_and_sync_singleton(chain_id, singleton_id: bytes, client: FullNodeRpcClient):
    ...
    while spent_block_index is None or spent_block_index > 0:
        coin_recrods = await client.get_coin_records_by_parent_ids(
            [fetch_coin_id],
            include_spent_coins=True,
            start_height=spent_block_index,
            end_height=spent_block_index + 1 if spent_block_index else None,
        )
        odd_coin_record = None
